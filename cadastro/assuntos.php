            ...
            <div id="layoutSidenav_content">
                <main>
					...
					<a class="btn btn-info" href="../adm/dist/cadastro.php"><i class="fa fa-angle-left"></i> Voltar</a>
                        <h1 class="mt-4"> Cadastro de Assuntos</h1>
							<?php
								if(isset($_GET['mess'])){
									if($_GET['mess']=='erro'){			 
							...
							<form name="form1" id="form1" method="POST" action="addAssunto.php" onsubmit="return validar()">
								<div class="form-group">
									<label>Disciplina</label>
									<select class="form-control" name="disciplina" id="disciplina" >
									  <option value="0" disabled selected>Informe a disciplina...</option>
										<?php
											$result = "select * from tb_disc";
											$escolas = mysqli_query($conexao, $result);

											while($row = mysqli_fetch_assoc($escolas)) { ?> 
												<option value="<?php echo $row['cod_disc'];  ?>"> <?php echo $row['descricao']; ?>
												</option> <?php
											}
										?>
									</select>
								...
								<div class="form-group">
									<label>Assunto</label>  
									<input   type="text" name="assunto" id="assunto" class="form-control" placeholder="Insira o assunto" >
								</div>
								<div class="form-group" >
